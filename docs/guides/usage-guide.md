...
- Docker イメージは未提供 — `plan.md §11.B` が v1.9 以降で追跡（当初 v1.1 → v1.6 と繰り下げ。v1.6 は `auto_router` / NIM / `--env-file` を先行出荷、v1.7 で PyPI 公開で onboarding 摩擦を解消、v1.8 で用途別 4 プロファイル + GLM/Gemma 4/Qwen3.6 公式化 + apply 自動化を出荷、Docker / launcher は v1.9 以降）。
...
- **空/意味不明の返信、200 ステータス** → [サイレント失敗症状 #1](./troubleshooting.md#3-ollama-初心者--サイレント失敗-5-症状-v07-c)（`num_ctx` 低すぎ）。
...
- **クラウドプロバイダ全部 401** → 症状 #5（`OPENROUTER_API_KEY` / `ANTHROPIC_API_KEY` が未 export — [troubleshooting §1-2](./troubleshooting.md#1-2-env-には-export-が必須)）。
- **`Header of type authorization was missing` 401** → env var が child process に届いていない ([troubleshooting §1-3 / §1-4](./troubleshooting.md#1-3-環境変数が子プロセスに届いているかの確認))。
- **Claude Code 上で挨拶が `Skill(hello)` 等に化ける** → バックエンドモデルの過剰ツール呼び出し ([troubleshooting §4-1](./troubleshooting.md#4-1-挨拶がツール呼び出しに化ける-llama-3-3-70b-系))。
- **`capability-degraded` ログ行** → 想定内の観測; [troubleshooting §2](./troubleshooting.md#2-ログの読み方とよくあるパターン) 参照。
...
完全なポリシー、脅威モデル、脆弱性報告経路: [`docs/security.md`](./security.md)。英語版は [`docs/security.en.md`](./security.en.md)。
