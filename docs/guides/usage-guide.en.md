...
- No Docker image shipped yet — `plan.md §11.B` tracks this for v1.9+ (originally v1.1 → v1.6, then re-scoped: v1.6 shipped `auto_router` / NIM / `--env-file`; v1.7 landed the PyPI publish to fix onboarding; v1.8 shipped use-case-aware 4 profiles + GLM/Gemma 4/Qwen3.6 official-tag promotion + `--apply` automation; Docker / launchers are v1.9+).
